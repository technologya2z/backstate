...
public class ApiInfoServiceImpl implements ApiInfoService {
    ...
    public Optional<ApiInfo> findOne(Long id) {
        ...
        List<Application> applicationListin = new ArrayList<>();
        List<Application> applicationListout = new ArrayList<>();

        if (apiInfo.get().getApplication() != null) {
            Optional<Application> application = applicationRepository.findById(apiInfo.get().getApplication().getId());
            apiInfo.get().setApplication(application.get());
        }
        for (ApiIn in : apiInfo.get().getApiIns()) {
            applicationListin.add(apiInService.findOne(in.getId()).get().getApplication());
        }
        for (ApiOut out : apiInfo.get().getApiOuts()) {
            applicationListout.add(apiOutService.findOne(out.getId()).get().getApplication());
        }
        apiInfo.get().setApplicationOuts(applicationListout);
        apiInfo.get().setApplicationIns(applicationListin);
        return apiInfo;
    }
