...
import { mapIdList, mapIdObjectList, findObjects, isEmptyObject } from 'app/shared/util/entity-utils';
...
import { getEntity, updateEntity, createEntity, reset, getEntities as getapplicationList } from './application.reducer';
...
export const ApplicationUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  useEffect(() => {
    if (!isEmptyObject(applicationEntity) && !isEmptyObject(applicationList)) {
      ...
      if (applicationEntity.subComponent) {
        const arr = applicationEntity.subComponent.split(',');
        const arrlist = [];
        for (let i = 0; i < applicationList.length; i++) {
          if (arr.includes(applicationList[i].name.trim())) {
  ...
  const saveEntity = values => {
    let sub = '';
    valueSubcomponent.map(otherEntity => (sub += otherEntity.name.trim() + ','));
      ...
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="cmspApp.application.home.createOrEditLabel" data-cy="ApplicationCreateUpdateHeading">
            <Translate contentKey="cmspApp.application.home.createOrEditLabel">Create or edit a Application</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
              ...
              >
                {applicationTypeValues.map(applicationType => (
                  <option value={applicationType} key={applicationType}>
                    {translate('cmspApp.ApplicationType.' + applicationType)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
